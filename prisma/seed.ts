...
const demoListings = [
  {
    ...
    houseRules: "No smoking. No pets. Please leave the space as you found it.",
  ...
  },
  {
    ...
    pricePerHour: 20,
    pricePerDay: 120,
    dayCheckInTime: "10:00",
    dayCheckOutTime: "20:00",
    houseRules: "Quiet hours after 9pm. Fire pit must be fully out before leaving.",
...
async function main() {
  ...
  for (const listing of demoListings) {
    ...
    const sharedFields = {
      ...
      dayCheckInTime: "dayCheckInTime" in listing ? listing.dayCheckInTime : null,
      dayCheckOutTime: "dayCheckOutTime" in listing ? listing.dayCheckOutTime : null,
      houseRules: "houseRules" in listing ? listing.houseRules : null,
  ...
  if (firstListing) {
    ...
    if (!existingBooking) {
      ...
      // Both sides reviewed — visible immediately since the double-blind
      // reveal condition (both submitted) is already met for this seed data.
      ...
      await prisma.hostReview.create({
        data: {
          ...
          renterId: renter.id,
          rating: 5,
          comment: "Great renter, communicated clearly and left the space in good shape.",
          visible: true,
        },
      });
    }
