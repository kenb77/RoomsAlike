...
export default function BookingCalendar({
  ...
  function bookingsOnDate(day: Date) {
    return booked.filter((b) => isSameDay(b.start, day));
  }
  ...
  function hostBookingsOnDate(day: Date) {
    return parsedHostBookings.filter((b) => isSameDay(b.start, day));
  }
      ...
      <div className="w-full">
        ...
        <div className="grid grid-cols-7 gap-1">
          {cells.map((day, i) => {
            ...
            const dayHostBookings = isOwnListing ? hostBookingsOnDate(day) : [];
  ...
  async function handleBook() {
    if (!startDateTime || !endDateTime || hours <= 0) {
      setError("Pick a date and a valid time range");
      return;
    }
  ...
  if (isOwnListing) {
    const dateBookings = selectedDate ? hostBookingsOnDate(selectedDate) : [];
    return (
        ...
        <div className="mt-4 space-y-3">
          ...
          {bookingsOnDate(selectedDate).length > 0 && (
            <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-2">
              Already requested/booked that day:{" "}
              {bookingsOnDate(selectedDate)
                .map(
                  (b) =>
                    `${b.start.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}–${b.end.toLocaleTimeString(undefined, { hour: "numeric", minute: "2-digit" })}`
                )
                .join(", ")}
            </div>
          )}
